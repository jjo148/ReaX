...
public:
    /**
     ...
     @param queueCapacity               Must be > 0. If you have to use CongestionPolicy::Allocate, use a large capacity, to make dynamic allocation on the audio thread as unlikely as possible. **The given `queueCapacity` may get rounded up to a different value.**
     @param timerIntervalInMilliseconds The interval in which the message thread polls the queue.
     @param dummy                       A dummy value that's used as an internal placeholder. You only need to pass this if T is not default-constructible.
